...
import { FilterDetails } from '../../contexts/filter.details';


const PriceSlider = ({products}) => {
  const priceValueDefault = {
      min:  products.reduce((acc,product) =>{
      ...
      } , products.filter((_, index)=> index < 1).map(product => product.price)[0]),
      max: products.reduce((acc,product) =>{
        if(product.price > acc){
          acc = product.price;
        }
        console.log(acc);
  ...
  // const {priceValueDefault} = useContext(FilterDetails);
